...
const QuoteDraft: React.FC<QuoteDraftProps> = ({ quote, scenarioLabel, setMode }) => {
        ...
        <div className="bg-white border-2 border-[#38BDF8] rounded-xl shadow-sm px-6 py-5 w-full relative">
          ...
          <div className="absolute top-3 right-3 z-10">
            ...
            >
              ...
              <svg className="w-5 h-5 text-[#0369A1]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          ...
          >
            <div className="flex flex-col flex-1 min-w-0 text-left">
              <span className="font-semibold text-[#0369A1] text-base truncate">{header.title}</span>
              <span className="text-xs text-gray-400 truncate">{scenarioLabel}</span>
            </div>
            <span className="px-2 py-1 rounded bg-[#E0F2FE] text-[#0369A1] text-xs font-semibold">{header.status}</span>
            <svg
            ...
            <div id={`quote-details-${header.quoteNumber}`}> 
              <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-1 mb-4 text-xs text-neutral">
                ...
                {header.priceProtectionExpiry && (
                  <div><span className="font-semibold text-light">Price Protection:</span> {header.priceProtectionExpiry}</div>
                )}
              </div>
                  ...
                  <tbody>
                    {items.map((item: typeof items[0], i: number) => (
                      <tr key={i}>
                        ...
                        <td className="py-1 text-right">{item.unitPrice.toLocaleString(undefined, { style: 'currency', currency: header.priceList.currency })}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex flex-col gap-1 mt-2 border-t border-[#E0F2FE] pt-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span>{summary && summary.subtotal !== undefined ? summary.subtotal.toLocaleString(undefined, { style: 'currency', currency: header.priceList.currency }) : ''}</span>
                </div>
                {summary && summary.discount !== undefined && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Discount</span>
                    <span>-{summary.discount.toLocaleString(undefined, { style: 'currency', currency: header.priceList.currency })}</span>
                  </div>
                )}
                {summary && summary.tax !== undefined && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Tax</span>
                    <span>{summary.tax.toLocaleString(undefined, { style: 'currency', currency: header.priceList.currency })}</span>
                  </div>
                )}
                <div className="flex justify-between items-center mt-1">
                  <span className="font-medium text-neutral">Total</span>
                  <span className="font-bold text-[#0369A1] text-lg">{summary && summary.total !== undefined ? summary.total.toLocaleString(undefined, { style: 'currency', currency: header.priceList.currency }) : ''}</span>
                </div>
              </div>
            </div>
          )}
